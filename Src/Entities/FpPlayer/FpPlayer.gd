...
func _ready() -> void:
	var _err = PlayerMovementSettings.connect("player_movement_settings_changed", Callable(self, "update_movement_settings"))
	if _err != OK:
...
func draw_movement_arrows() -> void:
	var interpolated_velocity = velocity_prev.lerp(velocity_now, Engine.get_physics_interpolation_fraction())
	...
	from += Vector3.UP * 0.1
	var interpolated_move_direction = move_direction_prev.lerp(move_direction_now, Engine.get_physics_interpolation_fraction())
...
func _physics_process(delta: float) -> void:
	...
	$FpHud.set_velocity(velocity_now.length())
	$FpHud.set_top_speed(fp_movement.top_speed)
	$FpHud.set_projection(fp_movement.projection)
	$FpHud.set_angle(rad_to_deg(fp_movement.velocity_move_direction_angle))
